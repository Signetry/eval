...
def test_the_generated_header_names_the_recipe_and_the_verifying_workflow():
    # The page is generated by `make leaderboard` and only *checked* by the workflow.
    # The header has to name both: the recipe, so a reader can reproduce the page, and
    # the workflow, so they know what fails when they edit it by hand.
    page = render_leaderboard(run_all(None))
    first = page.strip().split("\n")[0]
    assert "make leaderboard" in first
